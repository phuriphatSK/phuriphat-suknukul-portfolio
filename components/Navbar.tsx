...
export const Navbar = () => {
  ...
  return (
    <div className="z-50 fixed flex justify-center w-full text-white font-bold">
          ...
          {navLinks.map((link, index) => (
            <li key={index}>
              <Link
                href={link.path}
                onClick={closeNav}
                className="tranform hover:text-white/50 transition-all duration-300 ease-in-out"
              >
                {link.title}
              </Link>
            </li>
      ...
      <div
        onClick={toggleNav}
        className="md:hidden absolute right-5 top-5 z-50 border rounded text-white/70 border-white/70 p-2"
      >
